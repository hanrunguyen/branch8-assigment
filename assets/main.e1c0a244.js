import{$ as n,j as f}from"./vendor.a5b5ba75.js";n(document).ready(()=>{const o=["06:00-08:00","14:00-18:00","11:00-18:00","11:30\u201323:30","13:30\u201315:30"],_="https://fakerapi.it/api/v1/custom?title=city&image=image&description=text&date=dateTime&phone=phone&tag1=pokemon&tag2=pokemon&tag3=pokemon&address=streetAddress",p="https://fakerapi.it/api/v1/custom?title=city&image=image&description=text&date=dateTime&phone=phone&tag1=pokemon&tag2=pokemon&tag3=pokemon&address=streetAddress",v="https://fakerapi.it/api/v1/custom?title=city&description=text&date=dateTime&phone=phone&address=streetAddress",r="...";function d(s,a){return s=f.exports.trim(s),s.length>a?(s=s.substring(0,a-r.length),s.substring(0,s.lastIndexOf(" "))+r):s}function l(s){const t=Object.keys(s).filter(i=>i.includes("tag")).map(i=>s[i]);let c="";return t.forEach(i=>{c+=`<li>${i}</li>`}),c}function h(){fetch(_).then(s=>s.json()).then(s=>{const a=s.data;let e="";a.forEach((t,c)=>{const i=l(t);c<3&&(e+=`
              <article class="card">
